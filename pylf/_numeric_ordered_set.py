...
# collections.abc.Collection here. Because, collections.abc.Collection is added after
# Python3.5.
class NumericOrderedSet(
    collections.abc.Sized, collections.abc.Iterable, collections.abc.Container
):
    """A mutable set storing numeric value that remembers its elements insert order.
    Note that, this data structure has not implemented the full interfaces of
    collections.abc.MutableSet or collections.abc.Set. For simplicity, it only
    ...
    def __init__(self, typecode: str, privileged=None) -> None:
        ...
        The value of privileged must be within the typecode's range. The privileged can
        be successfully added more than one time to this data structure and appear more
        than one time in the ordered sequence."""
    ...
    def add(self, item) -> bool:
        ...
        privileged and already present. Otherwise, it will adds the item and returns
        True."""
